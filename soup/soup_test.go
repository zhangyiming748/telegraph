...
func TestProxy(t *testing.T) {
	file, err := os.OpenFile("exam.txt", os.O_APPEND|os.O_CREATE|os.O_RDWR, 0777)
	if err != nil {
		return
	}
	uri := "https://www.pornhub.com/model/cherryl-colle/videos?page=4"
	...
	t.Log(withProxy)
	root := HTMLParse(withProxy)
	as := root.FindAll("a")
	for _, a := range as {
		href := a.Attrs()["href"]
		if strings.Contains(href, "/view_video.php?viewkey") {
			fmt.Println("find ! ", href)
